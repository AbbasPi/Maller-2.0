...
function AddAddress() {
    ...
    const onSubmit = data => {
        axios.post(`${BASE_URL}/address`, data, { headers: {"Authorization" : `${TOKEN_STR.token.token_type} ${TOKEN_STR.token.access_token}`} })
    }
    ...
    !isAuth  ? navigate('/login')
        :
        (axios.get(`${BASE_URL}/address/city/all`).then((res)=>{
            setCity(res.data)
        }).catch((err)=>{
            console.log(err)
        }))
                    ...
                    <form className="bg-white px-6 py-8  rounded-xl shadow-md text-black w-full">
                        ...
                        <h1 className="mb-8 text-3xl text-center">Add Address</h1>
                         ...
                         border-red-400 text-red-700 px-4 py-3  rounded-xl relative`}
                                    ...
                                    <input className='block focus:outline-none focus:ring focus:ring-cyan-300 border active:border-black w-full p-3  rounded-xl mb-4'
                        ...
                        <div className='flex flex-col'>
                            <select className='float-left border p-2 font-medium bg-gray-100  rounded-xl mb-5'
                                ...
                                <option className='py-12 rounded-xl focus:outline-none focus:ring focus:ring-cyan-300' hidden >City</option>
                                {
                                    city.map((city)=>{
