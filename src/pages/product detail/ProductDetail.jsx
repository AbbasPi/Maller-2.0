...
function ProductDetail() {
    ...
    const addToWishlist = id => {
        ...
        if (!wish)
        {
            ...
            axios.post(`${BASE_URL}/wishlist/add/`, {id: id},
                    ...
                    <div className="lg:w-4/5 mx-auto flex flex-wrap">
                        ...
                        <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                            ...
                            <div className='flex justify-between'>
                                ...
                                {isAuth && <Rate setTrigger={setTrigger} productId={productId}/>}
                            </div>
                            <div dangerouslySetInnerHTML={{__html: product.description}}/>
                            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-200 mb-5">
                                <Link to={`/store/${product.vendor.id}`}
                                      className="text-2xl font-semibold hover:text-cyan-500 text-cyan-700 items-center tracking-widest">
                                                <span
                                                    className={'text-black font-medium'}>Store:</span> {product.vendor.name}
                                </Link>
                                <span
                                    className='bg-pink-50 rounded-md border p-1 ml-4 hover:bg-pink-100 hover:cursor-pointer -mb-2'>
                                    <Link
                            ...
                            <div className="flex">
                                    <span
                                        className="title-font font-medium text-2xl text-gray-900">{product.lowest_discounted < product.lowest ?
                                        <div>
                                ...
                                <button onClick={()=>addToWishlist(productId)}
                                    className={`rounded-full w-10 h-10 bg-gray-200 p-0 border-0 inline-flex
                                        items-center justify-center ml-4 hover:bg-gray-100
                                         ${wish ? 'text-cyan-700' : 'text-gray-500'} transition duration-300`}>
            ...
            1.06-1.06a5.5 5.5 0 000-7.78z"/>
                        ...
                        <div className='flex space-x-4 mt-4 rounded-2xl'>
                            {product.images.map((i) => {
                                return <button key={i} onClick={() => setMain(i.image)}>
                                    <img alt={product.images[0].alt_text}
                                         className={`w-28 h-24 w-full object-contain transition duration-100 object-center rounded border-2
                                          ${main === i.image && "hover:border-2 border-cyan-400"} `}
                                         src={i.image}/>
                                </button>
                            })}
                        </div>
