  ...
  useEffect(() => {
    const getProducts = () => {
      if (store_id !== undefined && store_id !== null) {
        axios
          .get(`${BASE_URL}/vendor/vendor/products/${store_id}`)
          .then((res) => {
      ...
      <div className="max-w-2xl mx-auto px-4 pt-12 pb-6 sm:px-6 lg:max-w-7xl lg:px-8">
        {!productsPage && (
          <div className="flex justify-between">
            ...
            <Link to={"/products"} className="text-2xl text-gray-900">
              Browse {">"}{" "}
            </Link>
          </div>
        )}
        <div className="mt-6 grid gap-y-24  gap-x-6 grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
          {pro.map((product) => (
            <div key={product.id} className="relative">
              <div
                className="bg-gray-100 hover:bg-white hover:shadow-xl transition-shadow duration-500
                                  hover:cursor-pointer ease-out w-full p-4 min-h-80 aspect-w-1 aspect-h-1
              ...
              <button
                ...
                className="border-2 rounded-xl
                                  ...
                                  p-2 hover:bg-[#39818d] hover:text-white hover:border-[#39818d]"
              >
                Add To Cart
              </button>
            </div>
