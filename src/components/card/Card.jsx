...
function Card({products, productsPage, store_id}) {
    ...
    return (<div className="bg-white">
        <div className="max-w-2xl mx-auto px-4 pt-12 pb-6 sm:px-6 lg:max-w-7xl lg:px-8">
            {!productsPage && <div className='flex justify-between'>
            ...
            <div className="mt-6 grid gap-y-24  gap-x-6 grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
                {pro.map((product) => (<div key={product.id} className="relative">
                    ...
                    <button onClick={() => {
                        addToCarts(product.id)
                                  ...
                                  p-2 hover:bg-[#39818d] hover:text-white hover:border-[#39818d]'>Add To Cart
                    </button>
                </div>))}
            </div>
        </div>
