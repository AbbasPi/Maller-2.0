...
function Checkout() {
  ...
  const onSubmit = (data) => {
    axios
      .put(`${BASE_URL}/address/${address[0].id}`, data, {
        headers: { Authorization: `${user.token_type} ${user.access_token}` },
      })
      .then(() => {
        getOrder();
  ...
  const checkout = () => {
    if (order && address) {
      axios
        .post(`${BASE_URL}/order/${address[0].id}/update_address`, null, {
          params: { order_pk: order[0].id, address_pk: address[0].id },
          headers: { Authorization: `${user.token_type} ${user.access_token}` },
        })
        .then(() => {
          axios
            .post(`${BASE_URL}/order/checkout`, null, {
              headers: {
    ...
    <div className="mt-32 font-[Poppins]">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-5 ">
        <div className="col-span-3">
          <h1 className="text-justify m-4 text-2xl font-[Poppins] ml-7 font-medium">
            Shipping Info
          </h1>
            ...
            <div className="container max-w-full mx-auto flex flex-col items-center justify-center ">
              <form className="bg-white px-6 rounded-xl text-black w-full">
                {address.map((address) => {
                  return (
                    <div key={address.id}>
                      ...
                      <h1 className="text-justify font-semibold p-1">
                        Address 1
                      </h1>
                      <input
                ...
                <div className="flex flex-col">
                  <div
                    className={`${
                      errors.city_id?.type === "required" ? "block" : "hidden"
                    } 
                                            bg-red-100 border border-red-400 text-red-700 px-4 py-3  rounded-xl relative`}
                    role="alert"
                  >
                  ...
                  >
                    <option className="py-12 rounded-xl" hidden>
        ...
        <div className="lg:col-span-2 col-span-3  mt-8 lg:mt-0 max-w-7xl bg-gray-100 rounded-xl">
          <p
            className={`${promoMsg === "" ? "hidden" : "block"} ${
              promoMsg === "Invalid Promo Code" ? "bg-red-600" : "bg-green-500"
            }
                     rounded-2xl p-2 text-xl text-white`}
          >
            {promoMsg}
          </p>
          <div className="text-left m-4 text-2xl flex justify-between border-b border-gray-400 uppercase pb-3">
            <input
              ...
              className={
                "p-3 lg:w-full w-60 focus:outline-none focus:ring focus:ring-cyan-300 border w-full p-3 rounded-2xl"
              }
            />
            <button
              onClick={() => addPromo()}
              className="bg-[#39818d] rounded-xl px-6 border border-white font-[Poppins] hover:bg-white
                         hover:text-[#39818d]  w-28 lg:ml-4 text-white hover:border-[#39818d]"
            >
              Apply
            </button>
          </div>
          ...
          <div className="flex justify-between border-b border-gray-400 uppercase pb-3 text-justify m-4 text-2xl">
            <h1 className="">Your Cart</h1>
            <h1>
              <Link className="hover:underline hover:opacity-70" to="/cart">
                Go To Cart
              </Link>
            </h1>
          </div>
                    ...
                    return (
                      ...
                      >
                        <div className="relative my-4">
                          ...
                          />
                          <span className="absolute -top-1 -right-2 bg-gray-900 text-white px-2 py-1 rounded-full">
                            {item.item_qty}
                          </span>
                        </div>
                        ...
                        <div className="w-10">
                          <h1 className="text-xl">${item.product.lowest}</h1>
                        </div>
                      </div>
                    );
                  })}
